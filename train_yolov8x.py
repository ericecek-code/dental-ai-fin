#!/usr/bin/env python3
"""YOLOv8x training for dental AI - targeting mAP50 >= 0.95."""
...
# Fix PyTorch 2.6+ weights_only issue for ultralytics
import torch
...
DATA_YAML = REPO_ROOT / "restorative-ai-bbox" / "data.yaml"
OUTPUT_DIR = REPO_ROOT / "runs" / "detect" / "train_v2"
...
def main() -> int:
    ...
    log("Starting YOLOv8x dental training (target mAP50 >= 0.95).")
    ...
    try:
        results = model.train(
            ...
            epochs=100,
            imgsz=640,
            batch=4,  # YOLOv8x needs smaller batch on 11GB VRAM
            patience=20,
            optimizer="AdamW",
            lr0=0.001,
            lrf=0.01,
    ...
    # Also run on test set if available
    test_metrics = {}
    try:
    ...
    results_file = REPO_ROOT / "training_results_v2.json"
